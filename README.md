...
- **태그는 축제명에서만 찾습니다.** 개요 본문까지 보면 거의 모든 소개글에 "먹거리", "공연" 이 들어가 경복궁 별빛야행이 먹거리로 잡히는 오분류가 쏟아집니다. `npx tsx scripts/check-tags.ts [태그명]` 으로 확인하세요.
...
- `tags` 를 직접 주면 자동 분류 대신 그 값을 씁니다. 사용 가능한 값: `먹거리`, `불꽃/야경`, `꽃/자연`, `문화/전통`, `음악/공연`, `전시/예술`, `봄`, `여름`, `가을`, `겨울`
